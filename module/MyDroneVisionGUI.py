...
class DroneVisionGUI:
    def __init__(self, drone_object, is_bebop, user_code_to_run, user_args, buffer_size=200, network_caching=200, fps=20, user_draw_window_fn=None):
        ...
        self.face_cascade = cv2.CascadeClassifier('/Users/Jonathan/Developing/lib/opencv/data/haarcascades/haarcascade_frontalface_default.xml')
        self.eye_cascade = cv2.CascadeClassifier('/Users/Jonathan/Developing/lib/opencv/data/haarcascades/haarcascade_eye.xml')
    ...
    def open_video(self):
        ...
        self.imagePath  = "/Users/Jonathan/Developing/Python/pyparrot-dev/images"
    ...
    def _start_video_buffering(self):
        ...
        # run user code
        # self.vlc_gui.drone_vision.run_user_code
    ...
    def _buffer_vision(self):
        ...
        if (self.vision_running):
            ...
            self.file = F"{self.imagePath}/visionStream_{datetime.datetime.now()}.jpg"
            ...
            if (img is not None):
                img, detect = self._face_detect(img)
                if detect:
                    # print(datetime.datetime.now().timestamp() - (self.last_detect + 5))
                    if (datetime.datetime.now().timestamp() - (self.last_detect + 1)) > 0:
                        self.last_detect = datetime.datetime.now().timestamp()
                        print(F"Face: {detect}")

                        if ((self.drone_object.sensors.flying_state != "emergency") &
                                (self.drone_object.sensors.flying_state == 'hovering')):
                                    self.drone_object.fly_direct(roll=0, pitch=-30, yaw=0, vertical_movement=0, duration=1)
                        self.drone_object.smart_sleep(0)
                else:
                    if ((self.drone_object.sensors.flying_state != "emergency") &
                            (self.drone_object.sensors.flying_state == 'hovering')):
                        self.drone_object.fly_direct(roll=0, pitch=0, yaw=10, vertical_movement=0, duration=1)
                    self.drone_object.smart_sleep(0)

                cv2.imshow('image', img)
                cv2.waitKey(1)
                cv2.imwrite(self.file.replace('.jpg','_cv2.jpg'), img)
                # print(self.file)
